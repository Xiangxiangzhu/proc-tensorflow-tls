...
class SumoSim:
    ...
    def gen_sim(self, no_addition=False):
        # create sim stuff and intersections
        # serverless_connect()
        # self.conn, self.sumo_process = self.server_connect()

        # port default 9000 + idx
        port = self.args.port + self.idx

        # sumo directory'/Users/tianzhaoming/sumo/bin/sumo'
        sumoBinary = checkBinary(self.sumo_cmd)
        # port = self.args.port + self.idx
        print("### configure file at ", self.cfg_fp, " will be loaded!")
        tls_dir = "networks/tls_addition.xml"
        if no_addition:
            ...
            self.sumo_process = subprocess.Popen([sumoBinary, "-c",
                                                  self.cfg_fp, "--remote-port",
                                                  str(port), "--additional-files", tls_dir, "--no-warnings",
                                                  "--no-step-log", "--random"],
                                                 stdout=None, stderr=None)
        ...
        # test vehicle generation and route planning here!
        # self.vehiclegen.run()
        # self.vehiclegen.run()
    ...
    def get_traffic_lights(self):
        ...
        # TODO_done: trafficlights may not be share the same id as it in junctions
        # todo: may remove those with only one green phase
        trafficlights = self.conn.trafficlight.getIDList()
        # junctions = self.conn.junction.getIDList()

        return set(trafficlights)
        ...
        # # then comment this and try it, if you can fix it you are the real MVP
    ...
    def create_tsc(self, rl_stats, exp_replays, eps, neural_networks=None):
        ...
        print("###### 11111111 is tsc_factory 11111111 ######")
        self.tsc = {tl: tsc_factory(self.args.tsc, tl, self.args, self.netdata, rl_stats[tl], exp_replays[tl],
                                    neural_networks[tl], eps, self.conn)
                    for tl in self.tl_junc}

    # used only for the generation of netdata
    def update_netdata(self):
        tl_junc = self.get_traffic_lights()
        tsc = {tl: TrafficSignalController(self.conn, tl, self.args.mode, self.netdata, 20, 30)
               for tl in tl_junc}
        ...
        for t in tsc:
            ...
            self.netdata['inter'][t]['all_lanes'] = tsc[t].all_lanes
